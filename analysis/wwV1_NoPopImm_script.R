...
for (i in N0:N2) {
  ...
  Rt_adj_synth[i] <- Rt_synth[i] ### produces zeros in Rt adj at the very beginning - need to change.
...
mod <- cmdstanr::cmdstan_model(stan_file = "models/wwV1_NoPopImm.stan")
fit <- mod$sample(
  data = data_list, 
  seed = 123, 
  chains = 1, 
...
hist(draws_df$mu, xlab = "", ylab = "", main = "Mu (R0)", 
     xlim = c(min(draws_df$mu), max(c(Rt_synth[1], draws_df$mu))))
