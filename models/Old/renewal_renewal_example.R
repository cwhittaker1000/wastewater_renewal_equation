## Tester Example for Renewal Equation Model WITHOUT Population Immunity
...
Rt_synth <- c(rep(2.5, 150), rep(1.2, 100), rep(0.9, 50), rep(0.5, 50))
Rt_adj <- vector(mode = "numeric", length = N2)
...
for (i in N0:N2) {
  ...
  Rt_adj[i] <- Rt_synth[i] # ((pop - cumm_sum[i]) / pop) * Rt_synth[i]
...
week_index <- c(rep(1, sum(Rt == unique(Rt)[1])), rep(2, sum(Rt == unique(Rt)[2])),
                rep(3, sum(Rt == unique(Rt)[3])), rep(4, sum(Rt == unique(Rt)[3])))
...
mod <- cmdstanr::cmdstan_model(stan_file = "models/base_NoPopImm.stan")
fit <- mod$sample(
  data = data_list, 
  seed = 123, 
  chains = 1, 
...
lines(colMeans(Rt)[1:N2], type = "l", ylab = "Reff", col = "black")
